...
void *incoming(void *arguments){
	...
	for(;;){
		...
		if ( args -> recvs == 0 ){
			printf(WHT "Server closed connection. Bye.\n" RESET);
			...
			if(strcmp(args -> buffer,"<<STOP_*_ACK>>") && strcmp(args -> buffer,"<<CLT_*_ACK>>")){
				
				if(!strncmp(args -> buffer, "<<", 2))
					printf( WHT "%s\n\n" RESET ,args -> buffer );
		...
		if ( !strncmp(args -> buffer, "\r<<", strlen("\r<<"))){
			sprintf(args -> buffer, "CLT_*_CHNG");
			send(args -> sock, args -> buffer, strlen(args -> buffer)+1, 0);
