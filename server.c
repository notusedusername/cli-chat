...
struct chat_rooms{
	...
	int choose[MAX_CLIENT];

	int change_count[MAX_CLIENT];
}rooms;
...
void *handler( void *arguments){
	...
	CHOOSE: if (chat_rooms(args -> sock[thread_no], message, in_message, thread_no, arguments) == 0){
	...
	sprintf(message, "Type '--help' for the control commands.");
	...
	while ( recvs != 0 ){
		...
		if(!strcmp(in_message, "--exit\n")){
			...
			else if( rooms.choose[thread_no] == 2 ){

				if(members(rooms.r2sock) > 1 ){			
			...
			sprintf(message, "\r<<%s changed  room ... This room is closed, exit or change room.>>\n", name);
			...
			for (int i = 0; i < 2; i++){
				...
				else if (rooms.r2sock[i] == args -> sock[thread_no]){
					rooms.r2sock[i] = 0;
					psock = 0;
					break;
				}
		...
		else if ( !strcmp(in_message, "CLT_*_CHNG")){

			printf("Clearing empty room...\n");
			for (int i = 0; i < 2; i++){
		...
		if(strcmp(in_message, "CLT_*_ACK") && psock != 0){
...
int main( int argc, char* argv[]){
	...
	while(bind(sock, (struct sockaddr *) &server, sizeof(server)) == -1){
